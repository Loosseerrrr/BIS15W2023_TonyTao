...
Very variable has its own column. Every observation has its own row. 

2. What is the difference between `select` and `filter`?  
Select operates between columns, filter works on rows. 
...
```{r}
bison_1980_1990 <- filter(new_bison, animal_yob >= 1980 & animal_yob <= 1990)
#bison_1980_1990 <- filter(new_bison, between(animal_yob, 1980, 1990))
```

5. How many male and female bison are represented between 1980-1990?
```{r}
table(bison_1980_1990$animal_sex)
```

6. Between 1980-1990, were males or females larger on average?
```{r}
males <- filter(bison_1980_1990 , animal_sex == "M")
females <- filter(bison_1980_1990, animal_sex == "F")
#male weight
mean(males$animal_weight)
#female weight
mean(females$animal_weight)
```
Between 1980-1990, males are larger on average. 
