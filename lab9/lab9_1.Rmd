...
```{r}
ggplot(data = homerange, aes(x=log10.mass, y=log10.hra)) + geom_jitter()
```

To add a regression (best of fit) line, we just add another layer.
```{r}
ggplot(data = homerange, aes(x=log10.mass, y=log10.hra)) + geom_jitter() + geom_smooth(method = lm)
```

### Practice
1. What is the relationship between log10.hra and log10.preymass? What do you notice about how ggplot treats NA's?
```{r}
ggplot(data = homerange, aes(x = log10.hra, y = log10.preymass)) + geom_point(na.rm = T)
```
